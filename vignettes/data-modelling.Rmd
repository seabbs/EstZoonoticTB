...
analysis_df <- df_with_ztb_filt_tb %>% 
  dplyr::select(-z_tb_dom_animal, -z_tb_wild_animal, 
                ...
                -cattle, -country, -g_whoregion, -z_tb_multi_year_study) %>% 
  dplyr::mutate(id = 1:n())
...
bootstrapped_df <- analysis_df %>% 
  ...
  mutate(tb_z_prop = ifelse(tb_z_prop == 0, 1e-5, 
               ...
               ## Add noise for variables that we have information about
                mutate(., prop_hiv = purrr::map2(prop_hiv, prop_hiv_hi, ~ 
                       ...
                       tb_z_prop = purrr::map2(tb_z_prop, tb_z_prop_se, ~ 
                                                 rtruncnorm(samples - 1, a = 1e-5, b = (1 - 1e-5), 
                                                            mean = .x, sd = .y))) %>% 
...
* All explanatory confounders and pairwise interactions, using penalised splines
...
models <- tibble(
  ...
  formula = c(
    ...
    "bf(tb_z_prop ~  year + prop_tb_ep + prop_hiv + prop_rural + cattle_per_head + 
    year:prop_tb_ep + year:prop_hiv + year:prop_rural + year:cattle_per_head + 
    ...
    "bf(tb_z_prop ~ t2(year, prop_tb_ep, prop_hiv, prop_rural, cattle_per_head))"
  )
) %>% 
...
split_df <- bootstrapped_df %>% 
    mutate_at(.vars = vars(year, prop_tb_ep, prop_hiv, prop_rural, cattle_per_head), scale) %>% 
    group_split(sample) %>% 
...
results <- models %>% 
  ...
  mutate(fit = purrr::map(formula, ~ fit_model(formula = as.formula(.))),
         ## Check convergence by looking at maximum Rhats across sampled fits. 
         convergence = map(formula, ~ check_convergence(fits)))
```
...
```{r, eval = FALSE}
## By year
plot_var_effect(spline_model, var = "year", 
                var_name = "Year")
## By Extra-pulmonary status
plot_var_effect(spline_model, var = "prop_tb_ep", x_is_prop = TRUE,
                var_name = "Proportion of TB cases with extra-pulmonary TB")
##  By HIV status
plot_var_effect(spline_model, var = "prop_hiv", x_is_prop = TRUE,
                var_name = "Proportion of TB cases with HIV")
## By the proportion of the population that is rural
plot_var_effect(spline_model, var = "prop_rural", x_is_prop = TRUE,
                var_name = "Proportion of the population that is rural")
##  By cattle per head
plot_var_effect(spline_model, var = "cattle_per_head", 
